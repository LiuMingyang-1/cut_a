"""Loader for prepared delta entropy span features."""
...
from cut_a_lab.methods.delta_entropy.contract import (
    DELTA_ENTROPY_INPUT_CONTRACT,
    DELTA_ENTROPY_VECTOR_FIELD_ALIASES,
)
from cut_a_lab.methods.delta_entropy.features import build_feature_names, coerce_feature_matrix
...
def _pick_vector_field(row: dict[str, Any], *, row_index: int) -> np.ndarray:
    for field_name in DELTA_ENTROPY_VECTOR_FIELD_ALIASES:
    ...
    raise ValueError(
        "Row "
        f"{row_index} is missing all accepted delta entropy vector fields: "
        f"{', '.join(DELTA_ENTROPY_VECTOR_FIELD_ALIASES)}."
    )


class DeltaEntropyMethod(BaseMethod):
    """Prepared span-level delta entropy feature method."""

    name = METHOD_NAME

    def input_contract(self):
        return DELTA_ENTROPY_INPUT_CONTRACT

    def load_feature_block(self, path: Path) -> FeatureBlock:
        rows = read_jsonl(Path(path))
        if not rows:
            raise ValueError(f"No rows found in delta entropy input file {path}.")
        ...
        for row_index, row in enumerate(rows):
            missing_required = [
                field_name for field_name in DELTA_ENTROPY_INPUT_CONTRACT.required_fields if field_name not in row
            ]
            if missing_required:
                raise ValueError(
                    f"Row {row_index} is missing required delta entropy fields: {', '.join(missing_required)}."
                )
