"""Loader for prepared ICR span features."""
...
from cut_a_lab.methods.icr.contract import ICR_INPUT_CONTRACT, ICR_VECTOR_FIELD_ALIASES
from cut_a_lab.methods.icr.features import build_feature_names, coerce_feature_matrix
...
def _pick_vector_field(row: dict[str, Any], *, row_index: int) -> np.ndarray:
    for field_name in ICR_VECTOR_FIELD_ALIASES:
        if field_name in row:
            ...
            return vector
    raise ValueError(
        f"Row {row_index} is missing all accepted ICR vector fields: {', '.join(ICR_VECTOR_FIELD_ALIASES)}."
    )


class ICRMethod(BaseMethod):
    """Prepared span-level ICR feature method."""

    name = METHOD_NAME

    def input_contract(self):
        return ICR_INPUT_CONTRACT

    def load_feature_block(self, path: Path) -> FeatureBlock:
        rows = read_jsonl(Path(path))
        if not rows:
            raise ValueError(f"No rows found in ICR input file {path}.")

        vectors: list[np.ndarray] = []
        records: list[SpanRecord] = []

        for row_index, row in enumerate(rows):
            missing_required = [field_name for field_name in ICR_INPUT_CONTRACT.required_fields if field_name not in row]
            if missing_required:
                raise ValueError(
                    f"Row {row_index} is missing required ICR fields: {', '.join(missing_required)}."
                )
